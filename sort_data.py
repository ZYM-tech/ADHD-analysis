...
from results import (
    ...
    FingerHolesResult,
    GrasshopperResult,
    ShapeColorInterferenceapeResult,
    ObjectsTrackingResult)
...
def objects_tracking(data):
    ...
    for record in data:
        ...
        if record['type'] == 'answer':
            ...
            answer = record['data']['index'] < select
            dbgprint("{:.3f}秒选择{}".format(ts, '正确' if answer else '错误'))
            if answer:
                result.right += 1
            else:
                result.wrong += 1
        ...
        if record["type"] == "end":
            ts = record["timestamp"] / 1000 - start_ts
            dbgprint("{:.3f}秒结束，共计{}个球，需要追踪{}个球".format(ts, total, select))
            print("===== {}选{}测试 =====".format(total, select))
            print("整体正确率：{}".format(result.accuracy))
...
def grasshopper(data):
    l_dict = {'1.20': "简单", '1.10': "普通", '1.05': "困难"}
...
def process_file(file, scene_name):
    with open(file, 'r', encoding="utf-8") as fp:
        ...
        if scene_name == "objects_tracking":
            objects_tracking(data)


if __name__ == "__main__":
    base_path = Path("/mnt/500g/ADHD")
    patient_path = base_path / 'Patient'
    control_path = base_path / 'Control'
    scenes = ['grasshopper', 'shape_color_interference', 'limb_conflict', 'finger_holes', 'objects_tracking']
    scene_names = {
        'grasshopper': "捉蚂蚱",
        'shape_color_interference': "形色干扰",
        'limb_conflict': "肢体冲突",
        'finger_holes': "戳洞",
        'objects_tracking': "多目标追踪"
    }
